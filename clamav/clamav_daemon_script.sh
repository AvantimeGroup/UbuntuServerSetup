...
while read FILE; do
    ...
    if [ "$?" == "1" ]; then
		...
		echo -e "33[31mMalware found!!!33[00m" "The file '$FILE' was moved to quarantine."
		if [ -f /usr/bin/notify-send ]; then
			notify-send -u critical "ClamAV Real Time" "Malware found!!! The file '$FILE' was moved to quarantine."
		fi
    fi
done